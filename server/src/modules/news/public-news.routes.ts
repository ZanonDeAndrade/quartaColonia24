...
export const registerPublicNewsRoutes = async (app: FastifyInstance, controller: NewsController) => {
  ...
  app.get<{ Params: { slug: string } }>('/slug/:slug', async (request, reply) => controller.getBySlug(request, reply));
  app.get<{ Params: { slug: string } }>('/:slug', async (request, reply) => controller.getBySlug(request, reply));
};
